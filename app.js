...
// Default prompts loaded from default-prompts.json
var DEFAULT_PROMPTS = [];
...
// Helper function to add default prompts for authenticated users
async function addDefaultPrompt() {
    if (!supabaseClient || !currentUser || DEFAULT_PROMPTS.length === 0) return;
    ...
    if (error) {
        console.error('Error adding default prompts:', error);
        return;
    }

    if (data) {
        data.forEach(row => {
            savedPrompts.push({ name: row.name, text: row.text, id: row.id });
        });
    }
}
